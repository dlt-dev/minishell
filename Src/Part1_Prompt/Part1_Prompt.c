...
static int len_prompt(t_shell *shell, t_valist *env, int *len_home, char *exit_status)
{ 
	int i;
	t_prompt *invite;
...
static char *create_prompt(t_shell *shell, t_valist *env ,t_prompt *invite)
{
	...
	if(exit_status == NULL)
		return(NULL);
	len_home = 0;
	p = malloc(sizeof(char ) * (len_prompt(shell, env , &len_home, exit_status) + 1));
	if(p == NULL)
		return(free(exit_status),  NULL);
	p[0] = '\0';
	ft_strcat(invite->color_exit, p);
	ft_strcat(exit_status,  p);
	ft_strcat("-> ", p);
